...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/03 13:56:35 by eblackbu          #+#    #+#             */
/*   Updated: 2020/05/03 13:56:43 by eblackbu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				get_arg_len(t_instr **instr, t_arg arg)
{
	...
	(*instr)->type.number > 8 && (*instr)->type.number != 13)
		return (IND_BYTE);
	return (DIR_BYTE);
}
...
void			make_one_string(t_instr **instr)
{
	...
	i = 0;
	num_arg = 0;
	if (!(((*instr)->bytestr = (char*)malloc(sizeof(char) \
				* (*instr)->full_size))))
		exit(-1);
	...
	{
		set_arg_to_string(instr, &i, (*instr)->args[num_arg]);
		num_arg++;
	}
}
